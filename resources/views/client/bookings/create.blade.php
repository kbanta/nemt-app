...
<div style="max-width: 680px;">
    ...
    <div class="card" style="padding: 32px;" x-data="bookingForm()">
        <form method="POST" action="{{ route('client.bookings.store') }}" id="booking-form" @submit="handleSubmit($event)">
            @csrf
            {{-- ── PATIENT NAME ─────────────────────── --}}
                ...
                <div style="background: linear-gradient(135deg, #0b1a2e, #1e3459); border-radius:12px; padding:20px 22px; color:#fff;">
                    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                        <div>
                            ...
                            <template x-if="includedMiles > 0">
                                <span>
                                    $<span x-text="basePrice.toFixed(2)"></span> base ·
                                    <span x-text="Math.max(0, distanceMiles - includedMiles).toFixed(1)"></span> extra mi ÷
                                    <span x-text="conditionMiles"></span> ×
            ...
            <div style="margin-bottom: 24px;">
                ...
                <div style="display:flex; flex-wrap:wrap; gap:10px;">
                    ...
                    <label style="flex:1; min-width:140px; display:flex; align-items:flex-start; gap:10px;
                        ...
                        <div>
                            ...
                            <p style="font-size:12px; color:#64748b;">Billed to your provider</p>
                        </div>
                    </label>
                ...
                <div x-show="paymentMethod === 'insurance'"
                    ...
                    <p style="font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:0.07em;
                            color:#16a34a; margin-bottom:14px;">Insurance Details</p>

                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">

                        <div>
                            <label class="form-label" style="font-size:12px;">Insurance Provider <span style="color:#dc2626;">*</span></label>
                        ...
                        <div>
                            <label class="form-label" style="font-size:12px;">Member / Policy ID <span style="color:#dc2626;">*</span></label>
                        ...
                        <div style="grid-column: span 2;">
                            <label class="form-label" style="font-size:12px;">Group Number <span style="color:#94a3b8; font-weight:400;">(optional)</span></label>
                    ...
                    <div style="display:flex; align-items:flex-start; gap:8px; margin-top:12px;
                        ...
                        <p style="font-size:12px; color:#166534; line-height:1.5;">
                            Our team will verify your coverage before the trip. You may be contacted if additional information is needed.
                        </p>
                    </div>
                </div>
            ...
            <button
                ...
                :disabled="distanceMiles <= 0 || submitting"
                :style="distanceMiles <= 0 || submitting ? 'opacity:0.5; cursor:not-allowed;' : ''"
            >
                ...
                <span x-text="submitting ? 'Please wait...' : (paymentMethod === 'online' ? 'Proceed to Payment' : 'Confirm Booking')"></span>
            </button>
            ...
            <p style="text-align:center; font-size:12px; color:#94a3b8; margin-top:10px;"
                ...
                Booking confirmed — we'll bill your insurance provider directly.
            </p>
            <p style="text-align:center; font-size:12px; color:#94a3b8; margin-top:10px;"
...
function bookingForm() {
    ...
    const component = {
        ...
        // ── Submit guard ─────────────────────────
        handleSubmit(e) {
            if (this.distanceMiles <= 0 || this.submitting) {
                e.preventDefault();
                return;
            }
            this.submitting = true;
            // Safety reset after 15s in case redirect fails
            setTimeout(() => { this.submitting = false; }, 15000);
        },
        ...
        onServiceChange() {
            ...
            if (!opt || !opt.dataset.base) {
                ...
                this.includedMiles = 0; this.conditionMiles = 1;
            } else {
        ...
        async calculateDistanceByCoords(fromCoords, toCoords) {
            ...
            try {
                ...
                this.distanceMiles = Math.round((summary.distance / 1609.344) * 10) / 10;
